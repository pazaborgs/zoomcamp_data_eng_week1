...
def ingest_data(url: str, engine, target_table: str, chunksize: int = 100000) -> None:
    ...
    # Criação do schema vazio no banco de dados
    ...
    print(f"Iniciando ingestão de dados para a tabela: {target_table}...")
    for df_chunk in tqdm(df_iter):
        df_chunk.to_sql(name=target_table, con=engine, if_exists="append")
        print(f"Lote inserido com sucesso ({len(df_chunk)} linhas)")
